document.addEventListener("DOMContentLoaded", () => {
  // Makes sure dom is loaded first before executing the rest of the script
  const width = 10;
  ...
  function startMultiplayer() {
    ...
    socket.on("player-number", (num) => {
      // listening to transmission titled player-number
  ...
  function dragDrop() {
    ...
    let lastVerticalIndex =
      parseInt(this.dataset.id) -
      selectedShipIndex * width +
      width * draggedShipLength;
    ...
    if (isHorizontal && !willWrapHorizontal) {
      ...
      for (let j = 0; j < draggedShipLength; j++) {
        let directionClass;
        if (j === 0) directionClass = "start";
        else if (j === draggedShipLength - 1) directionClass = "end";
        else directionClass = "middle";
        userSquares[
          parseInt(this.dataset.id) - selectedShipIndex + j
      ...
      for (let j = 0; j < draggedShipLength; j++) {
        ...
        userSquares[
          parseInt(this.dataset.id) - selectedShipIndex * width + width * j
...
// command + , from extensions to go into settings
// choose prettier as default formatter is settings if set to null
// extensions: prettier, bracket pair colorizer, auto rename tag, css peek (press control and click on class in html file), html css support (.name-of-class tab to complete)
